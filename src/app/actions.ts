...
export async function runAnalysis(
  ...
  | { success: true; data: IdentifyHourVarianceOutput }
...
> {
  try {
    ...
    const extractedResult = await extractTimesheetData({
      photoDataUri: timesheetDataUri,
    });
    if (
      !extractedResult ||
      !extractedResult.entries ||
      extractedResult.entries.length === 0
    ) {
      return {
        success: false,
        error:
          "Could not extract any data from the timesheet. The image might be unclear or not a valid timesheet.",
      };
    }
    ...
    const analysisResult = await identifyHourVariance({
      extractedData: JSON.stringify(analysisInput),
    });
    if (!analysisResult) {
      return {
        success: false,
        error: "Could not generate a summary for the timesheet data.",
      };
    }
    ...
    return {
      success: false,
      error: `Analysis failed. Please try again with a clearer image. Details: ${errorMessage}`,
    };
  }
}

export async function updateSummary(
  analysisData: IdentifyHourVarianceOutput
): Promise<
  { success: true; summary: string } | { success: false; error: string }
> {
  try {
    const analysisInput = {
      dailyHours: analysisData.dailyHours,
      totalHours: analysisData.totalHours,
      overtimeHours: analysisData.overtimeHours,
      missingHours: analysisData.missingHours,
      totalWorkingDays: analysisData.totalWorkingDays,
      totalRequiredHours: analysisData.totalRequiredHours,
    };

    // Get updated summary from the AI
    const analysisResult = await identifyHourVariance({
      extractedData: JSON.stringify(analysisInput),
    });
    if (!analysisResult) {
      return {
        success: false,
        error: "Could not generate an updated summary.",
      };
    }

    return { success: true, summary: analysisResult.summary };
  } catch (e) {
    console.error(e);
    const errorMessage =
      e instanceof Error ? e.message : "An unknown error occurred.";
    return { success: false, error: `Summary update failed: ${errorMessage}` };
  }
}
